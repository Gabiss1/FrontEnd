...
function ListarPeca() {
  ...
  const confirmDelete = () => {
    axiosInstance.delete(`/pecas/${selectedId}`)
      .then(response => {
        setRegistros(registros.filter(registro => registro.id !== setSelectedId));
          ...
          {registros.map(registro => (
            <tr key={registro.id}>
              ...
              <td class="action-column">
                <Link to={`/pecas/${registro.id}`} className="espaco_coluna">
